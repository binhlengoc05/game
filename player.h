...
struct Player {
    ...
    SDL_Rect rect;     // Hình chữ nhật để vẽ
    ...
    Player(){
        ...
        vy = 0;
        is_jumping = false;
        rect = { 0, 0, 40, 60 }; // Chiều rộng 40, chiều cao 60 pixel
    }
};
...
void updatePlayer(Player& player, Block** blocks, float deltaTime, float cameraOffsetY, float scrollSpeed) {
    ...
    player.vx = 0;
    if (keys[SDL_SCANCODE_A] || keys[SDL_SCANCODE_LEFT]) player.vx = -300.0f;
    if (keys[SDL_SCANCODE_D] || keys[SDL_SCANCODE_RIGHT]) player.vx = 300.0f;
...
void drawPlayer(Player player,Graphics graphics,float cameraOffsetY){
        ...
        SDL_SetRenderDrawColor(graphics.renderer, 255, 0, 0, 255); // Red player
        SDL_RenderFillRect(graphics.renderer, &player.rect);
